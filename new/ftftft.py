...
from singlestore import SingleStoreClient
...
# Create a single SingleStore connection object
...
"""
unique_keys_result = client.execute_query(query, database="information_schema")
unique_keys_dict = {}
for table in tables:
...
for table in tables:
    ...
    schema_result = client.execute_query(schema_query, database="prod_db")
    ...
    if not spark.catalog.tableExists(hive_table_name):
        # Create empty DataFrame with schema including as_of_dt
    ...
    # Execute SELECT query using SingleStoreClient
    records_to_delete_result = client.execute_query(select_query, database="prod_db")
