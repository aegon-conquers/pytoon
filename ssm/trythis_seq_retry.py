...
mysql_config_file = "/tmp/mysql.cnf"  # Use the path you tested locally

# Output directory in HDFS and local directory for loading
hdfs_output_dir = "hdfs://namenode:8021/user/spark/singlestore_load_data"
local_output_dir = "/tmp/singlestore_load_data"
...
# Step 6: Delete existing output directories if they exist
fs = spark._jvm.org.apache.hadoop.fs.FileSystem.get(spark._jsc.hadoopConfiguration())
hdfs_output_path = spark._jvm.org.apache.hadoop.fs.Path(hdfs_output_dir)
if fs.exists(hdfs_output_path):
    fs.delete(hdfs_output_path, True)

os.makedirs(local_output_dir, exist_ok=True)

# Step 7: Write to multiple files in HDFS
start_write_time = time.time()
rdd_formatted.saveAsTextFile(hdfs_output_dir)
write_time = time.time() - start_write_time
print(f"Writing files took {write_time} seconds")

# Step 8: Copy files from HDFS to local filesystem
start_copy_time = time.time()
subprocess.run(["hdfs", "dfs", "-get", f"{hdfs_output_dir}/*", local_output_dir], check=True)
copy_time = time.time() - start_copy_time
print(f"Copying files to local filesystem took {copy_time} seconds")

# Step 9: Verify generated files
local_files = [os.path.join(local_output_dir, f) for f in os.listdir(local_output_dir) if f.startswith("part-")]
print(f"Generated {len(local_files)} data files in {local_output_dir}")

# Step 10: Define a function to run a LOAD DATA command
def run_load_command(file_path, attempt=1):
    load_command = [
        "mysql",
        ...
        f"SET @@bulk_insert_buffer_size = 1024 * 1024 * 256"
    ]
    try:
        ...
        return False, str(e), e.stderr if e.stderr else ""

# Step 11: Define a function to log messages
def log_message(message):
    with open(log_file, "a") as f:
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        f.write(f"[{timestamp}] {message}\n")

# Step 12: Initial load attempt (sequential)
...
for file_path in local_files:
    ...
    success, stdout, stderr = run_load_command(file_path)
    if success:
        success_count += 1
...
# Step 13: Retry failed files
retry_round = 1
while failed_files and retry_round <= max_retries:
    print(f"Retry round {retry_round} for {len(failed_files)} failed files")
    new_failed_files = []
    for file_path, attempt_count in failed_files:
        time.sleep(retry_delay)  # Delay between retries
        print(f"Retrying {file_path} (attempt {attempt_count + 1})")
        success, stdout, stderr = run_load_command(file_path, attempt=attempt_count + 1)
        if success:
            success_count += 1
...
# Step 15: Clean up HDFS and local directories
if fs.exists(hdfs_output_path):
    fs.delete(hdfs_output_path, True)
import shutil
